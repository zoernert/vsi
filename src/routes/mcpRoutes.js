...
// MCP service status endpoint (with optional token in path)
...
// ============================================================================
// TOKEN-IN-PATH ENDPOINTS FOR MCP CLIENTS THAT CANNOT SEND HTTP HEADERS
// ============================================================================

// Middleware to extract token from path and set as Authorization header
const extractTokenFromPath = (req, res, next) => {
...
// MCP service status endpoint (with token in path)
router.get('/token/:token', extractTokenFromPath, authenticateToken, async (req, res) => {
    try {
        res.json({
            status: 'active',
            name: 'vsi-vector-store',
            ...
            usage: {
                ...
                transport: 'stdio',
                http: 'http://localhost:3000/api/mcp/',
                http_with_token_in_path: 'http://localhost:3000/api/mcp/token/YOUR_JWT_TOKEN/'
            },
            authentication: {
                method: 'token_in_path',
                note: 'JWT token is extracted from URL path and used for authentication'
            }
        });
    } catch (error) {
        res.status(500).json({
            status: 'error',
            message: error.message,
            authentication_method: 'token_in_path'
        });
    }
});

// MCP tools documentation endpoint (with token in path)
router.get('/token/:token/tools', extractTokenFromPath, authenticateToken, async (req, res) => {
    try {
        const tools = [
            {
                name: 'list_collections',
...
// MCP tool execution endpoint (with token in path)
router.post('/token/:token/call-tool', extractTokenFromPath, authenticateToken, async (req, res) => {
    try {
        const { name, arguments: args } = req.body;
        const token = req.params.token;
